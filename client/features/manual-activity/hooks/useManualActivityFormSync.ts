...
export function useManualActivityFormSync(options: {
  ...
  pendingRecent: { trackName: string; carName: string } | null;
  setPendingRecent: (v: { trackName: string; carName: string } | null) => void;
  ...
  useEffect(() => {
    ...
    const byCarName =
      pendingRecent.carName && pendingRecent.carName !== "—"
        ? cars.find((c) => c.name.trim().toLowerCase() === pendingRecent.carName.trim().toLowerCase())
        : null;
    if (byTrackName) form.setValue("trackId", byTrackName.id);
    if (byCarName) form.setValue("carId", byCarName.id);
    setPendingRecent(null);
  }, [pendingRecent, tracks, cars, form, setPendingRecent]);
}
