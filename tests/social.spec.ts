...
import { listHomeFeedSessionIds, uploadSessionJsonViaApi } from "./helpers/sessions";
...
test.describe("@social", () => {
  ...
  test("E1 — follow public user: challenge follows socialA → instant follow", async ({
    ...
    try {
      ...
      await gotoAuthenticated(page, challengeAuth, `/user/${socialAAuth.userId}`);
      await expect(page.getByRole("heading", { name: /E2E Social A/i })).toBeVisible();
      ...
      const status = await getFollowStatusViaApi(request, challengeAuth, socialAAuth.userId);
      expect(status.isFollowing).toBe(true);
    } finally {
      await unfollowUserViaApi(request, challengeAuth, socialAAuth.userId).catch(() => undefined);
    }
  });
  ...
  test("E4 — followers/following dialogs: /user/:socialA lists include B", async ({
    ...
    await expect(followersDialog.getByText(/E2E Social B/i)).toBeVisible({ timeout: 30_000 });
