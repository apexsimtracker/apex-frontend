...
const BAN_TARGET_TITLE = /E2E Ban Target Laguna GT3/i;
...
test.describe("@challenges-v2", () => {
  ...
  test("V2-1 — browse tabs show correct time labels (Upcoming / Live / Past)", async ({
    ...
    await gotoAuthenticated(page, auth, "/v2/challenges");
  ...
  test("V2-3 — challenge transitions Upcoming → Live → Past on detail page", async ({
    ...
    await gotoAuthenticated(
      page,
      viewerAuth,
      `/v2/challenge/${env.challengeTransitionId}`,
    );
    ...
    await gotoAuthenticated(page, viewerAuth, "/v2/challenges");
    await page.getByRole("button", { name: "Past", exact: true }).click();
  ...
  test("V2-4 — static cascade badges on profiles after seed disqualification", async ({
    ...
    await gotoAuthenticated(page, auth, `/v2/user/${standardAuth.userId}`);
    await expect(
    ...
    await gotoAuthenticated(page, auth, `/v2/user/${socialBAuth.userId}`);
    await expect(
      page.getByRole("button", {
  ...
  test("V2-5 — admin ban cascades ranks and updates profile badges", async ({
    ...
    try {
      ...
      await gotoAuthenticated(page, viewerAuth, `/v2/user/${proAuth.userId}`);
      await expect(
        page.getByRole("button", {
      ...
      await gotoAuthenticated(page, viewerAuth, `/v2/user/${standardAuth.userId}`);
      await expect(
        page.getByRole("button", {
          name: /P1 — GOLD — E2E Ban Target Laguna GT3/i,
        }),
  ...
  test("V2-6 — multi-win user opens challenge badge history modal", async ({
    ...
    await gotoAuthenticated(page, auth, `/v2/user/${proAuth.userId}`);
    ...
    await expect(
      page.getByRole("dialog").locator(
        `a[href="/v2/challenge/${env.challengePastId}"]`,
      ),
    ).toBeVisible();
    await expect(
      page.getByRole("dialog").locator(
        `a[href="/v2/challenge/${env.challengeCascadeId}"]`,
      ),
    ).toBeVisible();
  });
});
