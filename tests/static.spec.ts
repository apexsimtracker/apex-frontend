...
test.describe("@static", () => {
  ...
  test("K1 — public home CTAs link to signup and pricing", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { level: 1, name: /Your data\./i })).toBeVisible();
    await expect(page.getByRole("main").getByText("Sim racing performance hub")).toBeVisible();
  ...
  test("K2 — contact form submit success", async ({ page }) => {
    ...
    await expect(page.getByRole("status")).toContainText("We received your message");
  });
