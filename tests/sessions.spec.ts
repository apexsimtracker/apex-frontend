...
async function selectManualTrack(page: Page, trackPattern: RegExp): Promise<void> {
...
async function fillManualActivityBasics(page: Page): Promise<void> {
  ...
  await page.getByRole("group", { name: "Session type" }).getByRole("button", { name: "Practice" }).click();
...
test.describe("@sessions", () => {
  ...
  test("D1 — manual activity lifecycle: create → detail → edit → delete", async ({
    ...
    try {
      ...
      await expect(page.getByRole("heading", { name: "Log manual activity" })).toBeVisible();
      ...
      const createPost = page.waitForResponse(
        (res) =>
          res.url().includes("/api/sessions/manual-activity") && res.request().method() === "POST"
      );
      ...
      await expect(page).toHaveURL(new RegExp(`/sessions/${sessionId}$`), { timeout: 30_000 });
      await expect(page.getByText("Manual", { exact: true })).toBeVisible();
      await expect(page.getByText("Total Laps").locator("..").getByText("1")).toBeVisible();
      ...
      const updatePost = page.waitForResponse(
        (res) =>
          res.url().includes(`/api/sessions/${sessionId}`) && res.request().method() === "PUT"
      );
      ...
      await expect(page.getByRole("heading", { name: "Delete this manual activity?" })).toBeVisible();
      ...
      if (sessionId) {
        await deleteManualActivityViaApi(request, auth, sessionId).catch(() => undefined);
      }
    }
  });

  test("D2 — JSON upload (race podium): 8 laps, P2, qual P3", async ({ page, request }) => {
    const auth = await loginPersona(request, "standard");
  ...
  test("D3 — JSON upload (practice Monza): 7 laps", async ({ page, request }) => {
    ...
    await expect(page.getByText("Total Laps").locator("..").getByText("7")).toBeVisible();
    await expect(page.getByLabel("Session type: PRACTICE")).toBeVisible();
  });

  test("D4 — sessions library tabs: All / Telemetry / Manual", async ({ page, request }) => {
    ...
    const telemetryIds = await listActivityFeedSessionIds(request, auth, "telemetry");
  ...
  test("D5 — like + comment on uploaded session", async ({ page, request }) => {
    ...
    let likeBody = (await likeRes.json()) as { liked?: boolean; likeCount?: number };
    if (!likeBody.liked) {
      ...
      likeBody = (await likeAgainRes.json()) as { liked?: boolean; likeCount?: number };
    }
    ...
    expect(
      (commentsBody.comments ?? []).some((comment) => comment.body?.includes(commentText))
    ).toBeTruthy();
  });
  ...
  test("D8 — dedupe JSON uploads: duplicate noop then extended laps", async ({ page, request }) => {
    ...
    const beforeDuplicate = await getSessionDetailViaApi(request, auth, sessionId);
    ...
    await expect(page.getByText("Total Laps").locator("..").getByText("9")).toBeVisible();
  });
  ...
  test("D9 — weekend grouping on sessions feed", async ({ page, request }) => {
    ...
    expect(await getActivityFeedGroupingForSession(request, auth, staleUpload.sessionId)).toBe(
      "standalone"
    );
  ...
  test("D10 — edge case JSON uploads", async ({ page, request }) => {
    ...
    const shortDetail = await getSessionDetailViaApi(request, auth, shortLaps.sessionId);
    expect(shortDetail.lapCount).toBe(2);

    const trialDetail = await getSessionDetailViaApi(request, auth, timeTrial.sessionId);
    expect(trialDetail.sessionType).toBe("TIME_TRIAL");
    expect(trialDetail.lapCount).toBe(6);

    const offlineDetail = await getSessionDetailViaApi(request, auth, offlineTesting.sessionId);
    ...
    await expect(page.getByText("Total Laps").locator("..").getByText("2")).toBeVisible();
    await expect(page.getByRole("heading", { level: 1 })).toContainText(/Pace|Interlagos|Carlos/i);

    await gotoAuthenticated(page, auth, `/sessions/${timeTrial.sessionId}`);
    await expect(page.getByLabel("Session type: PRACTICE")).toBeVisible();

    await gotoAuthenticated(page, auth, `/sessions/${offlineTesting.sessionId}`);
    await expect(page.getByLabel("Session type: PRACTICE")).toBeVisible();
  });
});
