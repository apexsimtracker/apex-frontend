...
async function fetchAgentDownloadLink(
  ...
  return request.get(
    `${apiUrl}/api/agent/download-link?os=${encodeURIComponent(os)}`,
    {
...
function skipIfAgentDownloadUnavailable(
  status: number,
  ...
  if (status === 503) {
    test.skip(
      ...
      `Agent download unavailable (${body.code ?? status}): ${
        body.message ??
        "Configure R2 agent keys or AGENT_DOWNLOAD_PATH_* for local dev"
      }`,
    );
  }
}
...
test.describe("@agent-web", () => {
  ...
  test.describe("H1 — free lock on /agent", () => {
    test("standard user sees Pro lock and upgrade CTA", async ({
      page,
      request,
    }) => {
      ...
      await expect(
        page.getByRole("heading", { name: "Apex Agent" }),
      ).toBeVisible();
      await expect(
        page.getByText(
      ...
      await expect(
        page.getByRole("heading", { name: "Apex Pro Required" }),
      ).toBeVisible();
      await expect(
    ...
    test("standard user gets 403 on GET /api/agent/download-link", async ({
      request,
    }) => {
      const auth = await loginPersona(request, "standard");
  ...
  test.describe("H2 — Pro download", () => {
    test("proSeed receives download link from GET /api/agent/download-link", async ({
      request,
    }) => {
    ...
    test("proSeed starts download from /agent UI", async ({
      page,
      request,
    }) => {
      const auth = await ensureProSeedHasPro(request);
      ...
      const downloadButton = page.getByRole("button", {
        name: new RegExp(
          `Download ${AGENT_DOWNLOAD_FILENAMES[E2E_AGENT_OS].replace(".", "\\.")}`,
        ),
      });
      ...
      const downloadResponse = page.waitForResponse(
        (res) =>
          res.url().includes("/api/agent/download-link") &&
