...
async function selectCommunityCategory(page: Page, label: RegExp): Promise<void> {
...
test.describe("@community", () => {
  ...
  test("F1 — list, filter, and search discussions", async ({ page, request }) => {
    ...
    try {
      ...
      await expect(page.getByRole("heading", { name: "Sim Racing Community" })).toBeVisible();
      await expect(page.getByRole("link", { name: setupTitle })).toBeVisible({ timeout: 30_000 });
      await expect(page.getByRole("link", { name: guidesTitle })).toBeVisible();
      await expect(page.getByRole("link", { name: generalTitle })).toBeVisible();

      await selectCommunityCategory(page, /^General\b/);
      await expect(page.getByRole("link", { name: generalTitle })).toBeVisible();
      await expect(page.getByRole("link", { name: setupTitle })).toHaveCount(0);
      await expect(page.getByRole("link", { name: guidesTitle })).toHaveCount(0);

      await selectCommunityCategory(page, /^Setups\b/);
      await expect(page.getByRole("link", { name: setupTitle })).toBeVisible();
      await expect(page.getByRole("link", { name: generalTitle })).toHaveCount(0);
  ...
  test("F2 — create discussion in General and delete", async ({ page, request }) => {
    const auth = await loginPersona(request, "standard");
    ...
    try {
      ...
      await expect(page.getByRole("heading", { name: "Create New Discussion" })).toBeVisible();

      const createDialog = page.getByRole("dialog");
      await createDialog.getByRole("button", { name: "General", exact: true }).click();
      ...
      await expect(page.getByRole("heading", { name: "Delete this discussion?" })).toBeVisible();
      ...
      if (discussionId) {
        await deleteDiscussionViaApi(request, auth, discussionId).catch(() => undefined);
      }
    }
  });

  test("F3 — like, comment, and edit own discussion", async ({ page, request }) => {
    const auth = await loginPersona(request, "standard");
    ...
    try {
      ...
      const likePost = page.waitForResponse(
        (res) =>
          res.url().includes(`/api/community/discussions/${discussionId}/like`) &&
          res.request().method() === "POST" &&
          res.ok()
      );
      ...
      const commentPost = page.waitForResponse(
        (res) =>
          res.url().includes(`/api/community/discussions/${discussionId}/comments`) &&
          res.request().method() === "POST" &&
          res.ok()
      );
      ...
      await editDialog.locator("textarea").fill(`${E2E_BODY} Updated once for edit test.`);
  ...
  test("F4 — guest can read; write actions prompt login", async ({ page, request }) => {
    const auth = await loginPersona(request, "standard");
    ...
    try {
      ...
      await expect(page.getByText("Sign in to start a discussion.")).toBeVisible();
