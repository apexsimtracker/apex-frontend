  ...
  return (
    <Link to={`/v2/discussion/${id}`} className="block">
      ...
      >
        <div className="mb-2 flex items-center gap-2">
          ...
          >
            {showAvatar ? (
              ...
              <div
                className="flex size-6 shrink-0 items-center justify-center rounded-full bg-v2-surface-container-high font-v2-body text-[10px] font-medium text-v2-on-surface-variant"
                aria-label={`Avatar for ${authorDisplay}`}
              >
                {initials}
              </div>
            )}
            <p className="truncate font-v2-body text-xs font-bold text-v2-on-surface">
              {authorDisplay}
            </p>
          </button>
        ...
        <p className="mb-3 line-clamp-2 font-v2-body text-xs leading-relaxed text-v2-on-surface-variant">
          {excerpt}
        </p>

        <div className="flex items-center gap-3 font-v2-body text-[11px] text-v2-on-surface-variant">
          ...
          {wasEdited ? (
            <span className="ml-auto font-v2-body text-[10px] uppercase tracking-wide text-v2-on-surface-variant/70">
              Edited
            </span>
          ) : null}
        </div>
