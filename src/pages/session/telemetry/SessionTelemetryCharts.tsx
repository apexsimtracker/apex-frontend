...
type SessionTelemetryChartsProps = {
...
export default function SessionTelemetryCharts({
  traces,
  compareTraces,
  syncKey = SYNC_KEY,
    ...
    <div className="space-y-3">
      ...
      {show("speed") && (
        <ChartCard title="Speed" dense={dense}>
          <div data-testid="telemetry-driving-charts">
            <UPlotReact
              options={speedOpts}
              data={speedData}
              syncKey={syncKey}
...
function ChartCard({
  ...
  return (
    <div
      className={
        dense
          ? "flex h-full flex-col rounded-apex-lg border border-apex-outline-variant/10 bg-apex-surface-container-high p-3"
          : "flex h-full flex-col rounded-lg bg-apex-surface-container/60 p-2"
      }
    >
      <p className="mb-1 px-1 font-apex-body text-[10px] font-bold uppercase tracking-widest text-apex-on-surface-variant">
        {title}
      </p>
      {children}
    </div>
