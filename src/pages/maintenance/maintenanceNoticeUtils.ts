...
export const maintenanceBadgeBaseClassName =
  "inline-flex w-fit items-center rounded-apex-sm px-2.5 py-1 font-apex-body text-[10px] font-semibold uppercase tracking-wide";
...
export function maintenanceStatusBadgeClass(
  ...
  switch (status) {
    ...
    case "COMPLETED":
      return "border border-apex-success/30 bg-apex-success/10 text-apex-success";
    case "CANCELED":
      return "border border-apex-error/30 bg-apex-error/10 text-apex-error";
    case "SCHEDULED":
    default:
...
export function severityBadgeClass(severity: NotificationSeverity): string {
  switch (severity) {
    ...
    case "SUCCESS":
      return "border border-apex-success/30 bg-apex-success/10 text-apex-success";
    case "MAINTENANCE":
      return "border border-apex-outline-variant/20 bg-apex-surface-container-high text-apex-on-surface-variant";
    case "INFO":
    default:
      return "border border-blue-500/30 bg-blue-500/15 text-blue-200";
  }
