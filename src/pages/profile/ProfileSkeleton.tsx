...
function RaceRowSkeleton() {
  return (
    <div className="flex items-center gap-4 border-b border-apex-outline-variant/10 py-4 last:border-b-0">
      <div className="min-w-0 flex-1 space-y-1.5">
        <SkeletonBlock
          className={`h-3.5 w-32 max-w-full rounded-apex-sm ${sk}`}
        />
        <SkeletonBlock className={`h-2.5 w-16 rounded-apex-sm ${sk}`} />
      </div>
      <SkeletonBlock
        className={`hidden h-5 w-14 shrink-0 rounded-apex-sm sm:block ${sk}`}
      />
      <SkeletonBlock
        className={`hidden h-3.5 w-24 shrink-0 rounded-apex-sm md:block ${sk}`}
      />
      <SkeletonBlock className={`h-6 w-8 shrink-0 rounded-apex-sm ${sk}`} />
      <SkeletonBlock className={`h-3.5 w-14 shrink-0 rounded-apex-sm ${sk}`} />
    </div>
  );
}
...
export default function ProfileSkeleton({
  ...
  return (
    ...
    >
      {showBackLink && (
        <SkeletonBlock className={`h-4 w-14 rounded-apex-sm ${sk}`} />
      )}
      ...
      <section className="flex items-start gap-4 py-2">
        ...
        <div className="min-w-0 flex-1 space-y-3">
          <SkeletonBlock
            className={`h-7 w-44 max-w-[70%] rounded-apex-sm ${sk}`}
          />
          <div className="flex flex-wrap gap-3">
            <SkeletonBlock className={`h-3 w-[5.5rem] rounded-apex-sm ${sk}`} />
            <SkeletonBlock className={`h-3 w-[5.5rem] rounded-apex-sm ${sk}`} />
          </div>
          <div className="space-y-2 pt-1">
            <SkeletonBlock className={`h-3.5 w-full rounded-apex-sm ${sk}`} />
            <SkeletonBlock className={`h-3.5 w-[85%] rounded-apex-sm ${sk}`} />
          </div>
          <SkeletonBlock className={`h-7 w-24 rounded-full ${sk}`} />
        </div>
      </section>

      {showChallengeBadges && <ProfileChallengeBadgesSkeleton />}
      ...
      <section className="space-y-3">
        ...
        <div className="space-y-5 rounded-lg bg-apex-surface-container-low p-5">
          ...
          <div className="border-t border-apex-outline-variant/15 pt-5">
            <div className="flex h-36 items-end justify-between gap-1.5 sm:gap-2">
              {barHeights.map((heightClass, i) => (
                ...
                >
                  <SkeletonBlock
                    className={`w-full rounded-apex-sm ${heightClass} ${sk}`}
                  />
                  <SkeletonBlock className={`h-2.5 w-6 rounded-apex-sm ${sk}`} />
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="space-y-3">
        ...
        <div className="hidden lg:block">
          <div className="flex gap-4 border-b border-apex-outline-variant/15 pb-2">
            {["w-16", "w-10", "w-12", "w-8", "w-14"].map((w) => (
              <SkeletonBlock
                key={w}
                className={`h-2.5 ${w} rounded-apex-sm ${sk}`}
              />
            ))}
          </div>
          <RaceRowSkeleton />
              ...
              <div className="mb-3 flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1 space-y-1.5">
                  <SkeletonBlock
                    className={`h-3.5 w-32 max-w-full rounded-apex-sm ${sk}`}
                  />
                  <SkeletonBlock className={`h-2.5 w-20 rounded-apex-sm ${sk}`} />
                </div>
                <SkeletonBlock
                  className={`h-6 w-8 shrink-0 rounded-apex-sm ${sk}`}
                />
              </div>
              <div className="flex flex-wrap gap-2">
                <SkeletonBlock className={`h-5 w-14 rounded-apex-sm ${sk}`} />
                <SkeletonBlock className={`h-3.5 w-24 rounded-apex-sm ${sk}`} />
              </div>
            </div>
          ))}
        </div>
