    ...
    <div className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
          <div className="text-xs uppercase tracking-wider text-white/50">Tank capacity</div>
          <div className="mt-1 text-2xl font-semibold tabular-nums text-white">
            {fuel.tankCapacityL.toFixed(1)} L
          </div>
        </div>
        <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
          <div className="text-xs uppercase tracking-wider text-white/50">Avg per lap</div>
          <div className="mt-1 text-2xl font-semibold tabular-nums text-white">
            {fuel.avgFuelPerLapL != null ? `${fuel.avgFuelPerLapL.toFixed(2)} L` : "—"}
          </div>
        </div>
        <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
          <div className="text-xs uppercase tracking-wider text-white/50">Laps from full</div>
          <div className="mt-1 text-2xl font-semibold tabular-nums text-white">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
            ...
            <XAxis
              ...
              label={{ value: "Lap", position: "insideBottom", offset: -2, fill: "rgba(255,255,255,0.4)" }}
            />
            ...
            <Line
              ...
              dot={{ r: 2 }}
            />
            <Bar dataKey="fuelUsedL" name="Used (L)" fill="rgba(251,191,36,0.35)" radius={[4, 4, 0, 0]} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
    </div>
