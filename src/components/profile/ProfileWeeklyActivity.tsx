...
type ProfileWeeklyActivityProps = {
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-end justify-between">
        <p className="font-apex-body text-[9px] font-bold uppercase tracking-wider text-apex-on-surface-variant">
          {label}
        </p>
        <span className="font-apex-body text-xs font-bold text-apex-on-surface">
          {value}
          {suffix ? (
            <span className="text-[10px] font-normal text-apex-on-surface-variant">
              {" "}
              {suffix}
            </span>
          ) : null}
      ...
      <div className="h-1 w-full rounded-full bg-apex-surface-container-highest">
        <div
          className="h-full rounded-full bg-apex-primary transition-all duration-300"
          style={{ width: `${pct}%` }}
        />
      </div>
    </div>
            ...
            <div key={day} className="flex flex-1 flex-col items-center gap-2">
              <div className="relative flex h-28 w-full items-end justify-center">
                <button
                  ...
                  className={cn(
                    "group relative w-full cursor-pointer rounded-apex-sm transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-apex-primary/50",
                    value === 0 &&
                      "min-h-[2px] bg-apex-surface-container-highest/60",
                  )}
                  style={
                    value > 0
                      ? {
                          ...
                          background:
                            "linear-gradient(to top, hsl(var(--apex-primary)), hsl(var(--apex-primary) / 0.55))",
                        }
                      : { height: "2px" }
                  }
                ...
                >
                  {isHovered && value > 0 && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center rounded-apex-sm bg-apex-primary/20 p-1">
                      <p className="font-apex-body text-[10px] font-bold text-apex-on-surface">
                        {day}
                      </p>
              ...
              <span className="font-apex-body text-[10px] font-medium text-apex-on-surface-variant">
                {day}
              </span>
            </div>
          );
...
export function ProfileWeeklyActivity({
  weeklySnapshot,
  weeklyGoals,
  totalRaces,
  buckets,
}: ProfileWeeklyActivityProps) {
            ...
            <div className="relative flex size-20 shrink-0 items-center justify-center">
              ...
              >
                <circle
                  ...
                  r={DONUT_RADIUS}
                  fill="transparent"
                  stroke="hsl(var(--apex-surface-container-highest))"
                  strokeWidth="4"
                />
                <circle
                  cx="28"
              ...
              <div className="absolute flex flex-col items-center">
                <span className="font-apex-body text-sm font-bold text-apex-on-surface">
                  {formatHoursFromSeconds(trackTimeSec)}
                </span>
                <span className="font-apex-body text-[7px] font-bold uppercase tracking-tighter text-apex-on-surface-variant">
                  Total
                </span>
              </div>
            </div>
