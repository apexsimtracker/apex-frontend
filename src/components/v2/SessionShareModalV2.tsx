...
const ICON_SIZE = 32;
const ROW_CLASS =
  "flex w-full items-center gap-3 rounded-v2-sm border border-v2-outline-variant/20 bg-v2-surface-container-low px-3 py-2.5 text-left font-v2-body text-sm text-v2-on-surface transition-colors hover:bg-v2-surface-container";
...
export interface SessionShareModalV2Props {
  open: boolean;
...
export default function SessionShareModalV2({
  open,
  onOpenChange,
  shareUrl,
  shareText,
}: SessionShareModalV2Props) {
    ...
    >
      <section aria-labelledby="share-prefill-heading">
        <h3
          ...
          className="mb-2 font-v2-headline text-xs font-semibold uppercase tracking-wider text-v2-on-surface-variant"
        >
          Prefills in the app
        </h3>
        <nav
      ...
      <section aria-labelledby="share-no-prefill-heading">
        ...
        <p className="mb-3 font-v2-body text-xs leading-relaxed text-v2-on-surface-variant">
      ...
      <div className="border-t border-v2-outline-variant/15 pt-4">
        <button
          type="button"
          onClick={() => void copyToClipboard()}
          className={cn(
            ROW_CLASS,
            "justify-center font-medium hover:bg-v2-surface-container-high",
          )}
          aria-label="Copy session details and link to clipboard"
        >
          {copied ? (
            <>
              <Check className="size-4 text-v2-primary" aria-hidden />
              Copied
            </>
          ) : (
            <>
