...
import DashboardActivityCardV2, {
  type SessionPatch,
} from "@/components/v2/dashboard/DashboardActivityCardV2";
...
interface BundledActivityCardV2Props {
...
export default function BundledActivityCardV2({
  sessions,
  overflowCount,
  onSessionPatch,
  currentUser,
}: BundledActivityCardV2Props) {
  ...
  return (
    <div
      className="relative overflow-hidden rounded-2xl border border-v2-outline-variant/10 bg-v2-surface-container-low"
      role="region"
    ...
    >
      <div className="absolute left-2 top-1/2 z-20 -translate-y-1/2">
        <button
          ...
          className={`rounded-full border border-v2-outline-variant/20 bg-v2-background/80 p-1.5 backdrop-blur-sm transition-all ${
            currentIndex === 0
              ? "cursor-not-allowed opacity-30"
              : "hover:border-v2-outline-variant/40 hover:bg-v2-background"
          }`}
        >
          <ChevronLeft className="size-4 text-v2-on-surface" />
        </button>
      </div>
      <div className="absolute right-2 top-1/2 z-20 -translate-y-1/2">
        ...
        >
          <ChevronRight className="size-4 text-v2-on-surface" />
        </button>
      </div>

      <div className="p-2">
        <DashboardActivityCardV2
        ...
        {sessions.map((s, i) => (
          <button
            ...
            className={`size-1.5 rounded-full transition-all ${
              i === currentIndex
                ? "w-3 bg-v2-primary"
                : "bg-v2-on-surface-variant/40 hover:bg-v2-on-surface-variant/60"
            }`}
          />
        ))}
        {overflowCount > 0 &&
          (profileOwnerId ? (
            <Link
              to={`/v2/user/${encodeURIComponent(profileOwnerId)}`}
              onClick={(e) => e.stopPropagation()}
              className="ml-2 text-xs text-v2-on-surface-variant hover:text-v2-on-surface"
            >
              +{overflowCount}
            </Link>
          ) : (
            <span className="ml-2 text-xs text-v2-on-surface-variant/60">
              +{overflowCount}
            </span>
          ))}
      </div>

      <div className="absolute bottom-10 right-4 text-xs text-v2-on-surface-variant/60">
