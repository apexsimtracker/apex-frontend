...
import BundledActivityCardV2 from "@/components/v2/dashboard/BundledActivityCardV2";
...
export type ActivityFeedListV2Props = {
  items: ActivityFeedItem[];
  onSessionPatch?: (sessionId: string, patch: SessionPatch) => void;
  currentUser?: { id: string; avatarUrl?: string | null } | null;
};

export default function ActivityFeedListV2({
  items,
  onSessionPatch,
  currentUser,
      ...
      {items.map((item) => {
        ...
        return (
          <div
            ...
            className="overflow-hidden rounded-2xl border-2 border-v2-outline-variant/25 bg-v2-surface-container-low/40"
          >
            <WeekendGroupHeader
              group={item.group}
              className="mb-0 rounded-none border-0 border-b border-v2-outline-variant/20 bg-v2-surface-container-low/60"
            />
