...
export function AgentInstallRequirements({
  ...
  return (
    <section className="mb-8">
      <h2 className="mb-4 font-apex-headline text-xs font-bold uppercase tracking-widest text-apex-on-surface-variant">
        Before you install
      </h2>
      <div className="rounded-lg border border-[#2a2a2a] bg-apex-surface-container p-6">
        <p className="text-sm text-apex-on-surface-variant">
        ...
        <ul className="mt-5 grid gap-3 sm:grid-cols-3">
          {requirements.map((item) => (
            ...
            >
              <div
                className="flex size-9 shrink-0 items-center justify-center rounded-lg border border-[#2a2a2a] bg-apex-surface-container"
                aria-hidden
              >
                <item.icon className="size-4 text-apex-on-surface-variant" />
              </div>
              <div className="min-w-0">
                <p className="font-medium text-apex-on-surface">{item.label}</p>
                {item.description ? (
                  <p className="mt-1 text-xs leading-relaxed text-apex-on-surface-variant">
                    {item.description}
                  </p>
                ) : null}
              </div>
        ...
        {scopeNote ? (
          ...
          >
            <Info
              className="mt-0.5 size-4 shrink-0 text-apex-on-surface-variant"
              aria-hidden
            />
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-apex-on-surface">
                Platform scope
              </p>
              <p className="mt-1.5 text-sm leading-relaxed text-apex-on-surface-variant">
                {scopeNote}
              </p>
            </div>
          </div>
